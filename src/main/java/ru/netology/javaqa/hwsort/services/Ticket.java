...
public class Ticket implements Comparable<Ticket> { //сравнение билетов по умолчанию
    ...
    @Override
    public int compareTo(Ticket o) { //метод сортировки (встроенный алгоритм сортировки)
        if (price < o.getPrice()) {
            return -1;
        }
        if (price > o.getPrice()) {
